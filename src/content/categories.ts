// Categories du hub SEO Jeitinho — 18 catégories, regroupées en 5 piliers.
// Le regroupement en piliers (PILLARS) ne change AUCUNE URL existante :
// les routes restent /blog/categorie/:slug comme avant. Le champ `pillar`
// sert uniquement à organiser la navigation, le footer et la page /blog.
...
export const PILLARS: Pillar[] = [
  { slug: "decouvrir-rio", name: "Découvrir Rio" },
  { slug: "vivre-carioca", name: "Vivre carioca" },
  { slug: "manger-sortir", name: "Manger & sortir" },
  { slug: "organiser-sejour", name: "Organiser son séjour" },
  { slug: "rio-quotidien", name: "Rio au quotidien" },
];
...
export interface Category {
  ...
  pillar: string;            // slug d'un Pillar ci-dessus — pour le regroupement nav/footer
...
export const CATEGORIES: Category[] = [
  ...
  {
    slug: "vie-pratique-locale",
    name: "Vie pratique locale",
    title: "Le quotidien à Rio — Pharmacies, courses, argent, santé",
    description: "Ce que les guides touristiques ne disent jamais : pharmacies, supermarchés, distributeurs, santé, laveries — comment vivre le quotidien à Rio comme un local.",
    intro: "Ce n'est pas glamour, mais c'est ce qui vous évite les galères. Où faire ses courses, retirer de l'argent, se soigner, laver son linge — le vrai quotidien carioca, expliqué par ceux qui le vivent.",
    pillar: "rio-quotidien",
  },
];
...
/** Catégories groupées par pilier, dans l'ordre de PILLARS — pour la nav et le footer. */
export function getCategoriesByPillar(): { pillar: Pillar; categories: Category[] }[] {
  return PILLARS.map((pillar) => ({
    pillar,
    categories: CATEGORIES.filter((c) => c.pillar === pillar.slug),
  }));
}
